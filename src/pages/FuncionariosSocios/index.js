import React from "react";
import { useNavigation } from "@react-navigation/core"

import AvatarHome from "../../assets/icon-avatar-home.png"

import IconPlusOrange from "../../assets/icons/icon-plus-circle-orange.png"

import IconPlusCircle from "../../assets/icons/icon-plus-circle-white.png"

import {
  SafeAreaView,
  Text,
  View,
  TouchableOpacity,
  Image,
  ScrollView,
} from "react-native"
import Styles from "./style"

import Header from "../../components/Header"

export default function () {
  const navigation = useNavigation();
  return (
    <SafeAreaView>
      <ScrollView>
        <View style={Styles.container}>
          <Header nickName="Renata Mendes" source={AvatarHome} />
          {/* Seção de saldo e escolhas das contas de banco */}
          <View style={Styles.content}>
            <View style={Styles.titulo}>
              <Text style={Styles.textTitulo}>Funcionários & Sócios</Text>
            </View>
          
          </View>
          {/* Seção de Botoes flutuantes */}
          <View style={Styles.sectionButtons}>
            <View style={Styles.button}>
              <TouchableOpacity>
                <Image source={IconPlusCircle} style={Styles.iconCirclePlus} />
              </TouchableOpacity>
              <Text style={Styles.buttonText}>
                 Cadastrar{"\n"}Funcionários
              </Text>
            </View>
            <View style={Styles.button}>
              <TouchableOpacity>
                <Image source={IconPlusCircle} style={Styles.iconCirclePlus} />
              </TouchableOpacity>
              <Text style={Styles.buttonText}>Cadastrar{"\n"}Sócios</Text>
            </View>
          </View>
          {/* Seção de COnteudo de Funcionarios e Sócios */}
          <View style={Styles.sectionContent}>
            <View style={Styles.titleSection}>
             <Text style={Styles.textTitulo}>Funcionários</Text>
            </View>
            <View style={Styles.cardContainer}>

              <View style={Styles.produtoItem}>
                <View style={Styles.productContent}>
                  <View style={Styles.circleProdut}></View>
                  <Text style={Styles.textProduto}>
                    <Text style={Styles.productName}>Funcionário</Text>{'\n'}
                    VR e VT{'\n'}
                    Salário{'\n'}
                    Direitos Trabalhistas{'\n'}
                    Impostos{'\n'}
                  </Text>
                </View>
              </View>
              <View style={Styles.produtoItem}>
                <View style={Styles.productContent}>
                  <View style={Styles.circleProdut}></View>
                  <Text style={Styles.textProduto}>
                    <Text style={Styles.productName}>Funcionário</Text>{'\n'}
                    VR e VT{'\n'}
                    Salário{'\n'}
                    Direitos Trabalhistas{'\n'}
                    Impostos{'\n'}
                  </Text>
                </View>
              </View>

            </View>
            
          </View>

          <View style={Styles.sectionContent}>
            <View style={Styles.titleSection}>
             <Text style={Styles.textTitulo}>Sócios</Text>
            </View>
            <View style={Styles.cardContainer}>

              <View style={Styles.produtoItem}>
                <View style={Styles.productContent}>
                  <View style={Styles.circleProdut}></View>
                  <Text style={Styles.textProduto}>
                    <Text style={Styles.productName}>Sócio</Text>{'\n'}
                    VR e VT{'\n'}
                    Salário{'\n'}
                    Direitos Trabalhistas{'\n'}
                    Impostos{'\n'}
                  </Text>
                </View>
              </View>
              <View style={Styles.produtoItem}>
                <View style={Styles.productContent}>
                  <View style={Styles.circleProdut}></View>
                  <Text style={Styles.textProduto}>
                    <Text style={Styles.productName}>Sócio</Text>{'\n'}
                    VR e VT{'\n'}
                    Salário{'\n'}
                    Direitos Trabalhistas{'\n'}
                    Impostos{'\n'}
                  </Text>
                </View>
              </View>

            </View>
            
          </View>


        </View>
      </ScrollView>
    </SafeAreaView>
  );
}
